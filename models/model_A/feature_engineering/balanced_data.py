...
def create_balanced_dataset(input_file="raw_combined_data.parquet", output_file="balanced_dataset.parquet"):
...
def split_balanced_dataset_maintain_ratio(input_file="after_feature_engineering.parquet"):
    root = find_project_root()
    ...
    print(f"Train: {len(selected_train_pos)} positive, {len(selected_train_neg)} negative patients")
    print(f"Test: {len(selected_test_pos)} positive, {len(selected_test_neg)} negative patients")
