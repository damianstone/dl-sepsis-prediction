...
def remove_highly_correlated_features_global(df, threshold=0.85):
    ...
    upper_df = pd.DataFrame(upper, columns=corr_matrix.columns, index=corr_matrix.columns)
...
def filter_balanced_dataset_main():
    input_path = Path("dataset/XGBoost/feature_engineering/balanced_dataset.parquet")
    output_path = Path("dataset/XGBoost/feature_engineering/balanced_dataset_filtered.parquet")
    report_path = Path("dataset/XGBoost/feature_engineering/correlation_report.csv")
    ...
    df_physio_filtered, dropped, corr_report, corr_matrix = remove_highly_correlated_features_global(df_physio, threshold=0.85)
