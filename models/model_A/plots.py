...
def get_feature_importance(model, feature_names):
    ...
    with torch.no_grad():
        ...
        emb_importance = torch.mean(
            torch.abs(emb_weights), dim=1
        )  # shape: (input_dim,)
...
def plot_less_10_features(model, feature_names):
    ...
    # Sort features by importance
    sorted_features = dict(
        sorted(feature_importance.items(), key=lambda x: x[1], reverse=False)
    )
...
def save_plots(
    root, xperiment_name, loss_counter, y_test, y_probs, y_pred, model, feature_names
):
    ...
    for plot_func, args, filename in [
        ...
        (
            plot_less_10_features,
            (model, feature_names),
            "10_less_important_features.png",
        ),
