...
def vectorization(df, time_col="ICULOS", id_col="patient_id"):
    ...
    sepsis_labels = (
        df_original.groupby(id_col)["SepsisLabel"].max().rename("SepsisLabel")
    )
    ...
    df_vector = df_vector[
        [
            ...
            if not (col.startswith("SepsisLabel_") and col != "SepsisLabel")
        ]
    ]
    return df_vector
...
def clean_vectorized_output(df_vector):
    ...
    keep_dynamic = [
        col for col in df_vector.columns if any(kw in col for kw in stat_keywords)
    ]
