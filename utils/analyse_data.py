...
def summaryStatistics(df):
    ...
    print(
        "Average number of measurements per patient:", measurements_per_patient.mean()
    )
    print(
        "Median number of measurements per patient:", measurements_per_patient.median()
    )
    print("\n")

    df_grouped = df.groupby("SepsisLabel")[["HR", "Temp", "SBP"]].agg(
...
def nullCols(df):
    cols_to_analyse = [
        col for col in df.columns if col not in ["patient_id", "dataset"]
    ]
    ...
    plt.bar(
        missing_percent_sorted.index, missing_percent_sorted.values, color="#7393B3"
    )
...
def plot_density_of_actual_values(df, value_col="ICULOS"):
    patient_iculos_density = df.groupby(["patient_id", value_col]).apply(
...
def distributions(df, columns=None):
    ...
    for col in columns:
        ...
        ks_2samp(sepsis_data, non_sepsis_data)

        sns.kdeplot(sepsis_data, label="Sepsis", shade=True, color="#7393B3", alpha=0.5)
        sns.kdeplot(
            non_sepsis_data, label="Non-Sepsis", shade=True, color="#D55E00", alpha=0.5
        )
        ...
        plt.axvline(
            non_sepsis_data.mean(),
            color="grey",
            linestyle="--",
            linewidth=1.5,
            label="Non-Sepsis Mean",
        )
...
def plot_percentage_sepsis_grid(df):
    ...
    for i, col in enumerate(features):
        ...
        sepsis_percentage = (
            df_non_null[df_non_null["MeasurementBin"].isin(valid_bins)]
            .groupby("MeasurementBin")["SepsisLabel"]
            .mean()
            * 100
        )
        ...
        slope, intercept, r_value, p_value, std_err = linregress(
            bin_centers, sepsis_percentage
        )
        print(f"{col}:slope:{slope}, c:{intercept}")
        best_fit = slope * np.array(bin_centers) + intercept
        axes[i].plot(
            bin_centers, best_fit, "r--", label=f"Best Fit (slope={slope:.2f})"
        )
...
def plot_avg_missing_data_sepsis(df):
    ...
    avg_missingness = missing_data = missing_data = missing_data = (
        df.assign(missing=missing_data).groupby("SepsisLabel")["missing"].mean() * 100
    )
    plt.figure(figsize=(8, 6))
    sns.barplot(
        x=avg_missingness.index, y=avg_missingness.values, palette=["skyblue", "salmon"]
    )
...
def plot_combined_metrics(results):
    ...
    ax.bar(
        indices + 0.5 * width,
        df_norm["Wasserstein Distance"],
        width,
        label="Wasserstein Distance",
    )
    ax.bar(
        indices - 1.5 * width, df_norm["KL Divergence"], width, label="KL Divergence"
    )
...
def plot_feature_presence_by_prevalence(df: pd.DataFrame) -> None:
    ...
    feature_prevalence = patient_presence.mean(
        axis=0
    )  # average of 0/1 => fraction of 1's
    ...
    plt.xticks(
        ticks=np.arange(len(sorted_features)), labels=sorted_features, rotation=90
    )
